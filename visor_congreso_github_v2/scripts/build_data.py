...
def write_chunked_dataset(data_dir: Path, votes: list[dict], metadata: dict) -> None:
    ...
    for (year, month), items in sorted(by_month.items()):
        ...
        chunks.append({
            ...
            "file": rel,
            "votes": len(items),
            "member_votes": sum(len(v.get("members", [])) for v in items),
            "bytes": size,
...
def main() -> int:
    ...
    for idx, d in enumerate(iter_days(start, end), start=1):
        ...
        for url in vd.pdf_urls:
            ...
            try:
                ...
                vote_count += 1
                print(f"  OK {vote.get('id')} · diputats: {len(vote.get('members', []))}")
            except Exception as e:
    ...
    metadata = {
        ...
        "end": end.isoformat(),
        "note": "Generat automàticament des dels PDF oficials de detall de votació. Dades partides per mesos per evitar el límit de 100 MB de GitHub.",
    }
