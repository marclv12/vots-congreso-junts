...
function renderVoteArc(v){
  const rows = FLAT.filter(r=>r.voteId===v.id && r.party !== 'Altres');
  ...
  const summaryRows = Object.entries(summary).sort((a,b)=>partySort(a[0],b[0])).map(([p,s])=>[
    {html:partyBadge(p)}, num(s['Sí']||0), num(s['No']||0), num(s['Abstención']||0), num(s['No vota']||0), num(s.total||0), {html:`<strong>${escapeHtml(majorityVoteFromCounts(s))}</strong>`}
  ]);
  return `<article class="vote-card initiative-card detail-layout">
    <div class="detail-head">
      ...
      <div class="vote-total-pills">
        <span><b>${num(totals.si)}</b> Sí</span><span><b>${num(totals.no)}</b> No</span><span><b>${num(totals.abst)}</b> Abs.</span><span><b>${num(totals.novota)}</b> No vota</span>
      </div>
    </div>
    <div class="vote-visual-grid">
      <div class="hemi-card">
        <div class="hemi-title">Hemicicle de vot</div>
        <div class="hemi-wrap">
          <div class="majority-line"></div>
          <div class="majority-text">Majoria absoluta<br><strong>176</strong></div>
          ${dots}
        </div>
        <div class="hemi-legend">${Object.keys(summary).sort(partySort).map(p=>`<span>${dot(p)}${escapeHtml(p)}</span>`).join('')}</div>
      </div>
      <div class="group-card">
        <div class="hemi-title">Detall per partit/grup</div>
        ${table(['Partit/grup','Sí','No','Abs.','No vota','Total','Sentit'], summaryRows)}
      </div>
    </div>
  </article>`;
}
function partySummaryForVote(voteId){
  const out = {};
  for(const r of FLAT.filter(x=>x.voteId===voteId)){
...
function hemicycleDots(seats){
  ...
  for(let r=0;r<caps.length && idx<seats.length;r++){
    const cap = caps[r];
    const take = Math.min(cap, seats.length-idx);
    for(let j=0;j<take;j++){
      ...
      html += `<span class="seat-dot vote-${voteClass(seat.vote)}" title="${escapeAttr(seat.party)} · ${escapeAttr(seat.member)} · ${escapeAttr(seat.vote)}" style="left:${x}%;top:${y}%;background:${PARTY_COLORS[seat.party]||PARTY_COLORS.Altres}"></span>`;
    }
  }
  return html;
}
