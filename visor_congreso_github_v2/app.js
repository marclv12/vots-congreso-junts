...
function normalizePartyCount(c){
  return {
    'Sí': Number(c?.si ?? c?.['Sí'] ?? 0),
    'No': Number(c?.no ?? c?.['No'] ?? 0),
    'Abstención': Number(c?.abst ?? c?.['Abstención'] ?? c?.abstenciones ?? 0),
    'No vota': Number(c?.absent ?? c?.['No vota'] ?? c?.no_votan ?? 0),
    total: Number(c?.total ?? 0),
    sentit: c?.sentit || c?.sense || null
  };
}
function rowFromPartyVote(v, party, c){
  const n = normalizePartyCount(c);
  const vote = n.sentit || majorityVoteFromCounts(n);
  return {
    ...
    totals: v.totals || {}, member: '', group: party, party, vote, counts: n, telematic: false
  };
}
function expandSeatsFromSummary(summary){
  const seats = [];
  const add = (party, vote, n) => { for(let i=0;i<(n||0);i++) seats.push({party, vote, member:''}); };
  for(const [party,s] of Object.entries(summary).sort((a,b)=>partySort(a[0],b[0]))){
    add(party, 'Sí', s['Sí']);
    add(party, 'Abstención', s['Abstención']);
    add(party, 'No vota', s['No vota']);
    add(party, 'No', s['No']);
  }
  return seats;
}
...
async function loadData(){
  ...
  for(const v of (DATA.votes || [])){
    if(v.partyVotes){
      for(const [party,c] of Object.entries(v.partyVotes)) FLAT.push(rowFromPartyVote(v, party, c));
    } else {
...
function buildRuntimeIndex(){
  ...
  for(const [voteId, rows] of rowsByVote.entries()){
    ...
    for(const r of rows){
      ...
      if(r.counts){
        const n = normalizePartyCount(r.counts);
        summary[p] = {'Sí':n['Sí'],'No':n['No'],'Abstención':n['Abstención'],'No vota':n['No vota'],total:n.total || (n['Sí']+n['No']+n['Abstención']+n['No vota'])};
        majors[p] = r.vote || n.sentit || majorityVoteFromCounts(summary[p]);
      } else {
    ...
    for(const [p,s] of Object.entries(summary)) if(!majors[p]) majors[p] = majorityVoteFromCounts(s);
...
function hydrateFilters(){
  ...
  if(membersEl){
    ...
    membersEl.innerHTML = members.map(m=>`<option value="${escapeAttr(m)}"></option>`).join('');
  }
...
function filters(){
  return {
    q:norm($('q').value),
    member:$('memberFilter') ? norm($('memberFilter').value) : '',
    party:$('groupFilter').value,
...
function renderKpis(rows, votes){
  $('kpis').innerHTML = [['Votacions filtrades', num(votes.length)],['Registres de grup', num(rows.length)],['Partits/grups', num(new Set(rows.map(x=>x.party)).size)],['Sessions', num(new Set(votes.map(v=>`${v.date}_${v.session}`)).size)]].map(([l,v])=>`<div class="card"><div class="label">${l}</div><div class="value">${v}</div></div>`).join('');
}
...
function renderVoteArc(v){
  ...
  const seats = expandSeatsFromSummary(summary).sort((a,b)=>(VOTE_ORDER[a.vote]??9)-(VOTE_ORDER[b.vote]??9) || partySort(a.party,b.party));
  ...
  return `<article class="initiative-detail">
    ...
    <div class="initiative-grid">
      ...
      <div class="hemi-card clean-card">
        <div class="card-title-row"><h4>Votacions</h4><span>${num(seats.length)} vots</span></div>
        <div class="hemi-wrap">
...
function partySummaryForVote(voteId){
  ...
  const out = {};
  const v = INDEX?.votesById?.get(voteId);
  if(v?.partyVotes){
    for(const [p,c] of Object.entries(v.partyVotes)){
      const n = normalizePartyCount(c);
      out[p] = {'Sí':n['Sí'],'No':n['No'],'Abstención':n['Abstención'],'No vota':n['No vota'],total:n.total || (n['Sí']+n['No']+n['Abstención']+n['No vota'])};
    }
    return out;
  }
...
function hemicycleDots(seats){
  ...
  for(let r=0;r<caps.length && idx<seats.length;r++){
    ...
    for(let j=0;j<take;j++){
      ...
      html += `<span class="seat-dot vote-${voteClass(seat.vote)}" title="${escapeAttr(seat.party)} · ${escapeAttr(seat.vote)}" style="left:${x}%;top:${y}%;background:${VOTE_COLORS[seat.vote]||VOTE_COLORS['Sense vot']}"></span>`;
    }
  }
  return html;
}
...
function exportCsv(){
  const rows = filteredFlat();
  const headers = ['date','session','voteNumber','type','title','party','vote','sourcePdf'];
...
document.querySelectorAll('.tab').forEach(btn=>btn.addEventListener('click',()=>{
  ...
  for(const id of ['resum','grups','comparativa','iniciativa','votacions']) $('tab-'+id).classList.add('hidden');
